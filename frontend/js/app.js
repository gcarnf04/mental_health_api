...
// CORRECCIÓN: La URL debe ser relativa porque el frontend y el backend
// se sirven desde el mismo host y puerto.
const API_URL = "/analyze";
const STATUS_URL = "/get_status"; // Nueva URL para el estado
...
// Función auxiliar para obtener el ícono según el dispositivo
...
// Función auxiliar para cargar el dispositivo
async function loadExecutionDevice() {
    try {
        ...
        if (response.ok && data.device) {
            ...
            executionDevice.textContent = 'Servidor no responde';
        }
    } catch (error) {
        executionDevice.textContent = 'Servidor desconectado';
        console.error("Error al cargar el estado del dispositivo:", error);
    }
}
...
async function analyzeCase() {
  ...
  // Prepare payload
  const payload = {
    // CORRECCIÓN: Mapear al campo 'patient_text' esperado por FastAPI
    patient_text: text, 
...
// Initialize
toggleManualMode();
loadExecutionDevice(); // Carga el dispositivo al iniciar